...
class GWAS(pd.DataFrame):
    ...
    def subset_by_pos(self, chrom, start=None, stop=None):
        if start is not None and stop is not None:
            snps = self.loc[
                (self[GWAS.CHRCOL] == chrom)
                & (self[GWAS.BPCOL] >= start)
                & (self[GWAS.BPCOL] <= stop)
            ]
    ...
    def parse_gwas(cls, stream):
        ...
        try:
            df = pd.read_csv(
                stream, dtype=dtype_dict, delim_whitespace=True, compression="infer"
            )
        ...
        for column in GWAS.REQ_COLS:
            if column not in df:
                raise ValueError(
                    f"{column}-column not found in summary statistics at {stream}."
                )
